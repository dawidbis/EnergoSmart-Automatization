...
|---|---|---|---|
| `1_Scripts/py/.env` | Local Python | SMTP email + SQLite path for test data | No |
...
- Build `PAD_UpdateSQLDatabase` desktop flow with an **ODBC / SQL** action
  pointing at the local SQLite file (`2_Database/energosmart_history.db`)
...
3. Download the `.zip` → place in `4_Power_Platform_Solution/EnergoSmart_Solution.zip`
