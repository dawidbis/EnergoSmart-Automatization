...
|---|---|---|
| `setup.py` | `setup_odbc_bridge.bat` | One-time **RPA-bridge setup**: detects the **SQLite3 ODBC Driver** (downloads + installs it silently with elevation if missing) **and** sets the Windows env var **`ENERGOSMART_DB_PATH`** (absolute warehouse path the PAD desktop flow reads). `--check-only` reports driver status only. `install.bat` runs it automatically. |
| `setup_env.py` | `configure_email.bat` | Interactive wizard that writes `.env` (email/SMTP). Pre-fills defaults from any existing `.env`/`.env.example`, infers the SMTP server from your email domain (Gmail / Outlook / Microsoft 365), hides the password, backs up an existing `.env` to `.env.bak`, and can test the SMTP login (`--test`). |
| `generate_invoices.py` | `generate_documents.bat` | Generates a chosen number of typed test documents split by Cloud Flow path: **GREEN** (valid → auto-accept), **YELLOW** (zero/spike/drop → manual review), **RED** (flyer/blank → auto-reject). Files are named `GREEN_*` / `YELLOW_*` / `RED_*` so the other tools recognise them. |
...
| `clean.py` | `clean_artifacts.bat` | One cleaner for all test artifacts: generated `*.pdf`/`*.xlsx` (`--target files`), the **Microsoft 365** inbox via classic **Outlook COM** (`--target outlook`, since M365 blocks basic-auth IMAP), **Gmail** Sent + bounced-back mail via **IMAP → Trash** (`--target gmail`), the **monitor run-history** (`--target logs`), the **RPA-synced warehouse rows** (`--target rpa`, only `sector='Unknown'` test inserts), or `all`. **Dry run by default**; `--yes` deletes (the `.bat` asks first). Never touches the **seeded history** or source. |
| _(orchestrator)_ | `demo.bat` (repo root) | **Full guided run** for a fresh machine: install → configure email → build DB → generate → send. Calls the `1_Scripts\bat\` wrappers in order, asking before each step. |
| `healthcheck.ps1` | `warehouse_healthcheck.bat` | Read-only **warehouse health-check** (PowerShell + SQLite ODBC, the same driver the RPA uses): totals, anomalies, status breakdown and the most recent **RPA-synced** rows (`sector='Unknown'`). Confirms an Accepted reading reached the local DB. `-Recent N` to list more. |
...
Easiest path: run **`configure_email.bat`** (or `python setup_env.py`) — an interactive
...
|---|---|---|
| `DB_PATH` | generate, simulate, generate_invoices, clean (`--target rpa`), setup | `../2_Database/energosmart_history.db` |
| `OUTPUT_DIR` | simulate, generate_invoices, send, clean | `../3_Test_Documents` |
...
Run the Python commands from `1_Scripts/py/` and the PowerShell tools from `1_Scripts/ps/`
(or just use the `1_Scripts/bat/` wrappers, which handle the paths for you).
...
Or from the repo root: `1_Scripts\bat\build_database.bat` (builds the DB) then
`1_Scripts\bat\generate_documents.bat` (or the full `demo.bat`)
...
powershell -ExecutionPolicy Bypass -File ..\ps\healthcheck.ps1   # warehouse health-check
powershell -ExecutionPolicy Bypass -File ..\ps\monitor.ps1       # control panel (history + PAD)
powershell -ExecutionPolicy Bypass -File ..\ps\monitor.ps1 -Watch  # live read-only dashboard
```

Root entry points: **`demo.bat`** (full guided run) and **`monitor.bat`** (control
panel). The individual wrappers live in **`1_Scripts\bat\`**: `install.bat`, `setup_odbc_bridge.bat`,
`configure_email.bat`, `build_database.bat`, `generate_documents.bat`,
`send_documents.bat`, `run_tests.bat`, `warehouse_healthcheck.bat`, `clean_artifacts.bat`
