...
.PARAMETER DbPath
    ...
    .env, then ..\2_Database\energosmart_history.db relative to this script.
...
if (-not (Test-Path $resolved)) {
    ...
    Write-Host '        Build it first: build_database.bat (or generate_history_db.py).'
    exit 1
}
...
try {
    ...
    Write-Host '        with setup_odbc_bridge.bat (python setup.py), and run from 64-bit PowerShell.'
    exit 1
}

try {
    $total = [int](Get-Scalar $conn (Get-Sql 'count_total'))
    $clients = [int](Get-Scalar $conn (Get-Sql 'count_distinct_clients'))
    $anomalies = [int](Get-Scalar $conn (Get-Sql 'count_anomalies'))
    $synced = [int](Get-Scalar $conn (Get-Sql 'count_synced'))
    ...
    $statusRows = Read-Rows $conn (Get-Sql 'status_breakdown')
    if ($statusRows) {
    ...
    if ($synced -eq 0) {
        ...
        $recentRows = Read-Rows $conn ((Get-Sql 'recent_synced') -f $Recent)
